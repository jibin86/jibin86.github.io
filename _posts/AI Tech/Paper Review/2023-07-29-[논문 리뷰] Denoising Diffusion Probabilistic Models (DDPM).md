...
![Untitled](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled.png)
...
![Untitled 1](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 1.png)
...
- 모델이 하고자 하는 것:
    - 노이즈가 들어왔을 때, 노이즈를 어떻게 뺄 것인지 학습하여 $x_0$를 만든다.
        ...
        ![Untitled 5](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 5.png)
...
- 어떻게 이미지에 노이즈를 더해줄까 ⇒ 베타 스케줄링을 통해 노이즈의 정도를 조절하여 더해준다.
    - Beta scheduling
        ...
        ![Untitled 6](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 6.png)
...
- 위의 하나씩 노이즈를 추가하는 단계를 $x_0$에서 $x_t$까지 한번에 부여하면 다음과 같은 수식으로 나타낼 수 있다.
    ...
    ![Untitled 7](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 7.png)
...
- Loss
    ...
    - 아래의 Loss는 가우시안 분포들을 비교하는 KL-Divergence로 이루어져있다.
        ...
        ![Untitled 8](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 8.png)
        ...
        - x0가 주어진다면 아래와 같이 $q(x_{t-1}\|x_{t})$를 계산할 수 있다.
            ...
            ![Untitled 9](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 9.png)

<br>            

# 5. DDPM Loss

![Untitled 10](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 10.png)

<br>

## 5.1. VAE Loss

![Untitled 11](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 11.png)
...
- Regularizer on Encoder
    - KL-Divergence Loss는 잠재벡터 z의 분포를 N(0,1)와 비교하여 정규화해주는 역할을 한다.
        - KL Divergence
            ...
            ![Untitled 12](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 12.png)
...
![Untitled 13](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 13.png)
...
- 하지만 1000번의 step에 걸쳐 노이즈를 주입해보니까, Regularization term을 이용해서 $\beta_t$ 를 학습하지 않더라도 $x_T$ 이 아래와 같이, 표준정규분포와 매우 유사하게 형성된다는 것을 알게되었다.      
    ![Untitled 14](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 14.png)
...
- 이전에 Denoising 과정에서 **각 시점의 평균과 분산을 학습**해야한다고 했었다.
    ...
    ![Untitled 15](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 15.png)
...
- DDPM에서는 분산에 해당하는 $\sum_\theta(X_t,t)$ 을 상수화하였다.
    - 위의 Regularization term을 제외하는 부분에서, $\beta$ 는 미리 사전 정의된 스케줄에 따라서 결정되기 때문에 time dependent한 상수로 처리하기로 하였다. 따라서 분산은 상수화되어 학습할 필요가 없어졌다.
        - $\sigma^2_t = \tilde{\beta_t} =$ t 시점까지의 누적된 noise              
            ![Untitled 16](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 16.png)              
        
        ![Untitled 17](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 17.png)

<br>

- 분산을 학습할 필요가 없기 때문에 평균만 학습할 수 있는 Loss로 재구성하였다.      
    ![Untitled 18](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 18-1702523934160-119.png)

    
    - 둘의 mean function 간의 차이로 계산할 수 있다.      
        ![Untitled 19](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 19.png)

    
    하지만 위의 평균을 봤을 때, 우리가 모르는 값은 $\epsilon$ 이다.
    
    따라서 Loss function을 다음과 같이 $\epsilon$ 을 구하는 과정으로 수정할 수 있다.      
    ![Untitled 20](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 20.png)

    따라서 평균을 구성하는 입실론을 예측하는 것으로 간소화 할 수 있다.      
    ![Untitled 21](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 21.png)
    

    위의 수식에서 입실론에 대한 MSE와 노이즈와 관련된 계수가 붙어있음을 알 수 있다.
    
    그런데 DDPM은 여기서 계수 부분을 제거하여 Loss를 더 간소화 했다.      
    ![Untitled 22](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 22.png)
...
![Untitled 23](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 23.png)
...
![Untitled 24](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 24.png)
...
- x0를 예측하기 위한 reverse process의 모습
    - 큰 규모의 이미지 feature들은 초반에 모습을 드러내고, 이후에 뒷부분에서 세부적인 디테일들이 나타난다.          
        ![Untitled 25](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 25.png)          
        ![Untitled 26](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 26.png)

- 아래는 $x_0 ∼ p_θ(x_0\|x_t)$ 를 확률적으로 예측한 이미지이다.      
    ![Untitled 27](/../../images/2023-07-29-[논문 리뷰] Denoising Diffusion Probabilistic Models (DDPM)/Untitled 27.png)      
